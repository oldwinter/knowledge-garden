...
// node_modules/tiny-network/index.js
var require_tiny_network = __commonJS({
  "node_modules/tiny-network/index.js"(exports, module2) {
    var net = require("net");
    var http = require("http");
    var https = require("https");
    var assert = require("assert");
    var Stream = require("stream");
    var request = (method, url, payload, headers) => {
    ...
    var get4 = (url, headers) => request("get", url, "", headers);
    var post = (url, payload, headers) => request("post", url, payload, headers);
    var readStream3 = (stream) => {
      const buffer = [];
      return new Promise((resolve, reject) => {
        stream.on("error", reject).on("data", (chunk) => buffer.push(chunk)).on("end", () => resolve(Buffer.concat(buffer)));
      });
    };
    var getJSON = (url, headers) => Promise.resolve().then(() => get4(url, headers)).then(readStream3).then(JSON.parse);
    var ensureStatusCode2 = (expected) => {
    ...
    var stringify2 = (obj, { encode = encodeURIComponent } = {}) => Object.keys(obj).reduce((arr, key) => {
      const value = obj[key];
    ...
    module2.exports = {
      ...
      ensureStatusCode: ensureStatusCode2,
      cookieJar,
...
var en_default = {
  ...
  "This format will be used when available template variables contain date.": `This format will be used when available template variables contain date.`,
...
var zh_cn_default = {
  ...
  "movie content template desc 6": `{{image}}, {{url}}`,
  "Date format": `\u53C2\u6570\u65E5\u671F\u683C\u5F0F`,
  "This format will be used when available template variables contain date.": `\u8FD9\u4E2A\u683C\u5F0F\u662F\u7ED9\u4E0A\u9762\u83B7\u53D6\u5230\u7684\u53C2\u6570\u8FDB\u884C\u683C\u5F0F\u5316\u65F6\u663E\u793A\u7684\u5185\u5BB9 .`,
...
// douban/handler/DoubanMovieLoadHandler.ts
...
// douban/handler/DoubanAbstractLoadHandler.ts
var import_tiny_network = __toModule(require_tiny_network());
var DoubanAbstractLoadHandler = class {
  constructor(doubanPlugin) {
    this.doubanPlugin = doubanPlugin;
  }
  handle(url, editor) {
    Promise.resolve().then(() => (0, import_tiny_network.get)(url + "/", { headers: JSON.parse(this.doubanPlugin.settings.searchHeaders) })).then(import_tiny_network.readStream).then(esm_default2.load).then(this.parseSubjectFromHtml).then((content) => this.toEditor(editor, content));
  }
...
var Logger = class {
  error(e) {
    new import_obsidian2.Notice("Douban Plugin Error: " + e);
    return e;
  }
  warn(e) {
    new import_obsidian2.Notice("Obsidian Douban Plugin Warn: " + e);
    return e;
  }
  info(e) {
...
// douban/handler/DoubanOtherLoadHandler.ts
...
var DoubanEtractHandler = class {
  ...
  handle(searchExtract, editor) {
    ...
    var doubanSubjectHandlers = this._doubanSubjectHandlers.filter((h) => h.support(searchExtract));
  ...
  parseText(extract, settings) {
    ...
    var doubanSubjectHandlers = this._doubanSubjectHandlers.filter((h) => h.support(extract));
    if (doubanSubjectHandlers && doubanSubjectHandlers.length > 0) {
...
var DoubanFuzzySuggester = class extends import_obsidian3.FuzzySuggestModal {
  ...
  getItemText(item) {
    let text3 = item.type + "/" + item.score + "/" + item.title + "/" + item.cast;
    return text3;
  }
...
// douban/search/DoubanSearchModal.ts
...
var DoubanSearchModal = class extends import_obsidian4.Modal {
  ...
  onOpen() {
    ...
    const searchInput = new import_obsidian4.TextComponent(inputs).onChange((searchTerm) => {
...
var DoubanSettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian5.Setting(containerEl).setName(i18nHelper.getMessage("douban search url")).then((setting) => {
    ...
    new import_obsidian5.Setting(containerEl).setName(i18nHelper.getMessage("movie content template")).then((setting) => {
    ...
    new import_obsidian5.Setting(containerEl).setName(i18nHelper.getMessage("Person Name Language Mode")).then((setting) => {
    ...
    new import_obsidian5.Setting(containerEl).setName(i18nHelper.getMessage("Date format")).then((setting) => {
      setting.addMomentFormat((mf) => {
        setting.descEl.appendChild(createFragment((frag) => {
          frag.appendText(i18nHelper.getMessage("This format will be used when available template variables contain date."));
          frag.createEl("br");
    ...
    new import_obsidian5.Setting(containerEl).setName(i18nHelper.getMessage("Array Spilt String")).setDesc(i18nHelper.getMessage(`string to join between array type, such as author, actor`)).addText((textField) => {
...
var import_tiny_network2 = __toModule(require_tiny_network());
...
var ensureStatusCode = (expected) => {
  if (!Array.isArray(expected))
    expected = [expected];
  return (res) => {
    const { statusCode } = res;
    if (!expected.includes(statusCode)) {
      new import_obsidian6.Notice(`Request Douban failed, Status code must be "${expected}" but actually "${statusCode}"`);
    }
    return res;
  };
};
...
var Searcher = class {
  static search(searchItem, doubanSettings) {
    return Promise.resolve().then(() => (0, import_tiny_network2.get)(doubanSettings.searchUrl + searchItem, JSON.parse(doubanSettings.searchHeaders))).then(ensureStatusCode(200)).then(import_tiny_network2.readStream).then(esm_default2.load).then(SearchParserHandler.parseSearch).then(log.trace);
  }
};
...
var DoubanPlugin = class extends import_obsidian7.Plugin {
  putToEditor(editor, extract) {
    return __async(this, null, function* () {
      ...
      var content = this.doubanEtractHandler.parseText(extract, this.settings);
      if (content) {
  ...
  getDoubanMovieTextForActiveFile(editor) {
    return __async(this, null, function* () {
  ...
  geDoubanMovieTextForSearchTerm(editor) {
    return __async(this, null, function* () {
  ...
  onload() {
    return __async(this, null, function* () {
      ...
      this.addCommand({
        ...
        editorCallback: (editor) => this.getDoubanMovieTextForActiveFile(editor)
      });
      this.addCommand({
        id: "search-douban-and-input-current-file",
        name: i18nHelper.getMessage("search douban and import to current file"),
        editorCallback: (editor) => this.geDoubanMovieTextForSearchTerm(editor)
      });
